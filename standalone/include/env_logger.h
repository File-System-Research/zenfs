...
#ifndef AQUAFS_ENV_LOGGER_H
#define AQUAFS_ENV_LOGGER_H
...
class EnvLogger : public Logger {
 public:
  EnvLogger(Env* env, InfoLogLevel log_level = InfoLogLevel::ERROR_LEVEL)
      : Logger(log_level),
        env_(env),
  ...
  void Logv(const char* format, va_list ap) override {
    ...
    for (int iter = 0; iter < 2; iter++) {
      ...
      assert(p <= limit);
      {
        ...
        // if (now_micros - last_flush_micros_ >= flush_every_seconds_ *
        // 1000000) {
        //   FlushLocked();
        // }
        puts(std::string(base, p - base).c_str());
      }
      if (base != buffer) {
        delete[] base;
      }
 ...
 private:
  ...
  // mutable std::mutex mutex_;  // Mutex to protect the shared variables below.
